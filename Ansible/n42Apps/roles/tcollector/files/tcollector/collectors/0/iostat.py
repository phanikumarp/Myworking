    ...
    while True:
        ...
        for line in f_diskstats:
            ...
            if len(values) == 14:
                # full stats line
                for i in range(11):
                    print("%s%s %d %s dev=%s appname=%s" 
                          % (metric, FIELDS_DISK[i], ts, values[i+3], device,appname))
                ...
                if ret:
                    ...
                    print("%s%s %d %.2f dev=%s appname=%s" 
                          % (metric, "svctm", ts, svctm, device,appname))
                    print("%s%s %d %.2f dev=%s appname=%s"
                          % (metric, "await", ts, await, device,appname))
                    print("%s%s %d %.2f dev=%s appname=%s"
                          % (metric, "util", ts, float(util/1000.0), device,appname))

            elif len(values) == 7:
                # partial stats line
                for i in range(4):
                    print("%s%s %d %s dev=%s appname=%s"
                          % (metric, FIELDS_PART[i], ts, values[i+3], device,appname))
            else:
