...
#library(quantreg)
options(digits = 8)
#metadata<-fromJSON(file = "/home/tushar/newsvc.json",method = "C")
args = commandArgs(trailingOnly=TRUE)
metadata = fromJSON(args)
setwd("/opt/")
#make sure to add check for load metric data download is done or not.
#####################################################################################################
...
calcscore<-function(x){
  if(length(x[[1]])>9 & length(x[[2]])>9){
      q1<-quantile(x[[1]],c(0.05,0.95))
      q2<-quantile(x[[2]],c(0.05,0.95))
      n1<-abs(q1[1]-q2[1])
      n2<-abs(q1[2]-q2[2])
      d1=q1[1]+q2[1]
      d2=q1[2]+q2[2]
      if((n1==0 && d1==0) && (n2==0 && d2==0)){
        ...
        d2<-abs(q1[2]-q2[2])*100/(q1[2]+q2[2])
        if(d2<5){
          return(100)
        }else{
          return(0)
        ...
        d1<-abs(q1[1]-q2[1])*100/(q1[1]+q2[1])
        if(d2<5){
          return(100)
        }else{
          return(0)
...
analyse<-function(res1,res2,name,metricType){
...
#       iname=paste("/home/tushar/svc/analysis/thursday/rawdata/",name,".jpeg",sep="")
#       jpeg(filename = iname,width = 1080,height = 720,units = "px")
#       par(mfrow=c(2,1))
#       hist(res1$data$value)
#       hist(res2$data$value)
#       dev.off()
      if(nrow(ver1)==0 || nrow(ver2)==0){
        ...
        if(length(matchingBuckets)>0){
              ...
              w1<-lapply(data,calcscore)                                              #
               
              
#               iname=paste("/home/tushar/svc/analysis/thursday/bucketdata/",name,".jpeg",sep="")
#               jpeg(filename = iname,width = 1080,height = 720,units = "px")
#               par(mfrow=c(2,1))
#               hist(data[[1]][[1]])
#               hist(data[[1]][[2]])
#               dev.off()
              ...
              if(length(w)>0){
                ...
                scores1<-sapply(w1,"[[",1)                                            #
                finalscores<-mapply(function(x,y){return(max(x,y))},scores,scores1,SIMPLIFY = T)           #
                json<-list(metricName=name,metricType=metricType,score=mean(finalscores),error="No error",
...
while(count<1){
  ...
  if(loadindex %in% index){
    ...
    data.not.found1<-version1[index]
    metric.not.found1<-metricType[index]
    version1<-version1[-index]
    version2<-version2[-index]
    metricType<-metricType[-index]
    shift<-which(index<loadindex)
    if(length(shift)>0){
      loadindex<-loadindex-length(shift)
    }
  }
  ...
  if(loadindex %in% index){
    ...
    data.not.found2<-version2[index]
    metric.not.found2<-metricType[index]
    version1<-version1[-index]
    version2<-version2[-index]
    metricType<-metricType[-index]
   shift<-which(index<loadindex)
  ...
  if(exists("data.not.found1")){
    data.not.found<-data.not.found1
    metric.not.found<-metric.not.found1
    rm(data.not.found1)
    rm(metric.not.found1)
  }
  if(exists("data.not.found2") && exists("data.not.found")){
    data.not.found<-c(data.not.found,data.not.found2)
    metric.not.found<-c(metric.not.found,metric.not.found2)
    #data.not.found<-unique(data.not.found)
    rm(data.not.found2)
    rm(metric.not.found2)
  }else if(exists("data.not.found2")){
    data.not.found<-data.not.found2
    metric.not.found<-metric.not.found2
    rm(data.not.found2)
    rm(metric.not.found2)
  }
  ...
  metricType<-metricType[-loadindex]
    ...
    error<-paste("Could not find load metric!!!Comparison abandoned.",sep="")
    results<-list(score=NA,error=error)
    count=count+1
    break
  }
...
#               }
      results<-mapply(FUN = analyse,version1,version2[order(matchingMetrics)],names(version1),metricType,SIMPLIFY = F)
# ,startTime1,startTime2,endTime1,endTime2,
      count=count+1
      print(results)
    }
...
if(exists("data.not.found")){
  results<-c(results,mapply(function(x,y,naam){return(list(metricName=naam,metricType=y,score=NA,
            error="Zero data points downloaded for this metric."))},data.not.found,metric.not.found,
            names(data.not.found),SIMPLIFY=F,USE.NAMES=T))
}
# output<-list(canultary_output=list(comparisionId=metadata$service$comparisionID,serviceName=metadata$service$serviceName,
...
POST(url = "http://localhost:8161/n42-cas-services/resources/svc/saveSVCResults",body=output,encode = "json")
print("DB saving ")


#write("~/result.json",x = output)
#tryCatch(p<-POST(url = "http://localhost:8080/n42-cas-services/resources/svc/saveSVCResults",
#                 body=output,encode = "json",verbose()),warning=function(w){print(w)},error=function(e){print(e)})
